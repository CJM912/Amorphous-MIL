...
class MyDataset(data.Dataset):
    def __init__(self, libraryfile='', transform=None):
        ...
        for i,name in enumerate(lib['slides']):
            slide = Image.open('positives/%s.png'%name)
...
class OrigDataset(data.Dataset):
    def __init__(self, libraryfile='', transform=None):
        ...
        for i,name in enumerate(lib['slides']):
            ...
            sys.stdout.flush()
            # slides.append(Image.open('negatives/%s'%name))
            # slides.append(Image.open('RGB/%s'%name))
            slides.append(Image.open('E:/dataset/mathematics/heatmap/force_4096/alldata/%s'%name))
            #Flatten grid
            g = lib['grid'][i]
            grid.extend(g)
